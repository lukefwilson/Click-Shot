...
-(void)createThumbURL:(NSString*)urlString
    ...
    UIImage *image = [[SDImageCache sharedImageCache] imageFromDiskCacheForKey:urlString]; //REOMVED BY LUKE WILSON 5/16/14 to fix image orientation issue
    ...
    if (image) {
        ...
        dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void){
            ...
            generator.appliesPreferredTrackTransform = YES; // ADDED BY LUKE WILSON 5/16/14
